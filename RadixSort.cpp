...
inline bool init()
{
    ...
    else
    {
        ...
        gWindow = SDL_CreateWindow("Radix Sort",
...
void fillRectangle(SDL_Renderer *renderer, SDL_Rect *rectangle, std::vector<long> &v, int delay) {
    for (size_t a = 0, b = 0; a < SCREEN_WIDTH / 2; a++, b += 2)    {
        rectangle[a] = {static_cast<int>(b), static_cast<int>(SCREEN_HEIGHT - v.at(a)), 2, static_cast<int>(v.at(a))};
...
void fillRectangle(SDL_Renderer *renderer, SDL_Rect * rectangle, std::vector<long>& v) {
    for (int i = 0, j = 0; i < SCREEN_WIDTH / 2; ++i, j += 2)
    {
        rectangle[i] = {j, static_cast<int>(SCREEN_HEIGHT - v.at(i)), 2, static_cast<int>(v.at(i))};
        SDL_RenderFillRect(renderer, &rectangle[i]);
    }
}

void fillRectangle(SDL_Renderer *renderer, SDL_Rect * rectangle, std::vector<long>& v, long least) {
    ...
    for (long i = 0, j = 0; i < SCREEN_WIDTH / 2; ++i, j += 2)
    {
        ...
        rectangle[i] = {static_cast<int>(j), static_cast<int>(SCREEN_HEIGHT - v.at(i)), 2, static_cast<int>(v.at(i))};
        SDL_RenderFillRect(renderer, &rectangle[i]);
    }
}
